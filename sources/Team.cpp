...
Team::Team(Character *leader) :m_teammates{nullptr}, m_numsOfTeammate(0), m_leader(leader) {
    ...
    if(leader ==nullptr)  throw std::invalid_argument("Error: NULL argument\n");
    
    if(leader->m_inTeam) throw std::runtime_error("Error: Character is already in a team\n");
...
void Team::add(Character *teammate) {
    if (teammate == nullptr) throw std::invalid_argument("Error: NULL argument\n");

    if (m_numsOfTeammate == 10) throw std::runtime_error("The team is already full\n");

    if(teammate->m_inTeam) throw std::runtime_error("Character is already in a team\n");
...
void Team::attack(Team *againtTeam) {
    ...
    if (againtTeam == nullptr)  throw std::invalid_argument("Error: NULL argument\n");
    
    if (!m_leader->isAlive()) updateLeader();

    if (m_leader == nullptr) throw std::runtime_error("All the team is already dead");
    

    Character *closet_character = closetTarget(againtTeam);

    if (closet_character == nullptr)  throw std::runtime_error("All the enemy team is dead");
    ...
    for (i = 0; i < MAX_TEAMMATES*2; i++) {
        ...
        if (closet_character == nullptr) return;

        const auto &member = *m_teammates[i % 10];
        if(m_teammates[i % 10] == nullptr || member.isAlive() == 0)  continue;
